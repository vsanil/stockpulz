...
|---|---|---|---|
| **deferred** | **Rotate FIVE credentials** — PAT `ghp_vkMR…` (`admin:org`, widest blast radius), PAT `ghp_3MCR…`, cron-job.org key, Render key, and **`CRON_SECRET` LAST** (needs all 17 cron-job.org URLs updated, `#`→`%23`, or a morning delivery is lost to a silent 401). 🔕 **The owner WAIVED the 09-28 date on 2026-09-25: "dont worry about rotation". Do NOT re-raise it on a schedule — surface it only if a credential is exposed or the owner asks.** The standing trade is unchanged and known: a permanent credential in 18 places with nothing forcing a rotation. | OWNER | the owner re-opens it |
...
| **clears 2026-09-28** | 🔴 **THE CANARY IS RED BY DESIGN — DO NOT CHASE IT (owner: "leave it, it will clear itself", 09-25).** Two checks, neither a fault in the app. (1) `selfheal.healthy`: exactly 3 failed self-heal runs, ALL dated 2026-09-20 with `event: push` — the signature of GitHub refusing to parse `self_heal.yml` after a duplicate-YAML-key edit (cause fixed that night, `e4f6a46`). Red on `len(bad)>=3` in a 7-day window; `since` is a bare DATE with `created=>=` (INCLUSIVE), so 09-27 still counts them and it clears on the **09-28** run. (2) `runs.silent_failures`: the 09-25 morning run logged a `Connection reset by peer` on the earnings check and skipped it — correct per-feature degradation, picks still delivered. 26-hour window on a full timestamp, so expect it red on 09-26 too, clearing 09-27 absent a fresh blip. 🚨 **Do NOT widen either window, add either to `OWNER_ONLY_CHECKS`, or 'fix' the earnings retry to green a status line** — that is gating a real detector for convenience, the exact move these gates exist to stop. | WATCH | the 09-28 canary run |
...
- **🔴 The fix I first proposed was WRONG, and checking before building is what caught it.** "Memoize reads per request" duplicates `config_manager._gist_read_cache` (20 s, process-wide, invalidated by all four write paths) — duplicate reads inside one request were never the cost. Read the funnel before adding a cache in front of it.
- **Fix: `webhook._prefetch_admin_reads()`** warms the existing cache for every file in `_ADMIN_PREFETCH` **concurrently** (ThreadPoolExecutor), so the builders hit memory. No builder signature changes. **Must run AFTER `mark_feedback_read()`** — that write invalidates `feedback.json`, and prefetching first would cache the pre-write copy. Each fetch is isolated so one failing file cannot take the dashboard down.
- **The guard that matters: `test_every_file_admin_data_reads_is_in_the_prefetch_set`** wraps the funnel during a real `/admin/data` call and diffs against the tuple — a new builder reading a new file FAILS instead of silently reintroducing a serial round trip. 3 mutations verified failing (call removed, a file dropped, made serial).
- **🔴 Measurement traps, three in one hour:** (1) sandboxed `curl` showed 4–5 s first hits that were MY PROXY's DNS/TLS on `*.onrender.com` — already recorded, still cost three tool calls; **any Render latency claim is measured UNSANDBOXED first.** (2) My instrumentation counted 21 funnel calls, then a second layer counted 0 — because **conftest's autouse fixture REPLACES `_load_gist_file` with an in-memory store**, so the harness cannot time backend round trips at all; it can only count funnel calls. Two instruments disagreeing means the instrument is wrong, not the code. (3) An "idle threshold" experiment (5/20/45/75 s gaps, all fast) is what disproved the cold-server story — **falsify the hypothesis before measuring the cause.**
- **Separate finding, fixed in the same commit: the daily `analysis: refresh engine findings` commit REDEPLOYED the web service every night** at ~00:00 UTC for a markdown-only change the service never reads. Now `[skip render]` — Render-specific, GitHub Actions still run. Guard: `TestDocsOnlyCommitsDoNotRedeploy`, incl. that `[skip ci]` is NOT used (it would silence the test workflow too).
- **🔴 THE FIX SKIPPED ITS OWN DEPLOY.** The commit that ADDED `[skip render]` to the analysis job also *described* it — its body said "Now [skip render]." — and **Render scans the whole message**, so `ea21ca2` never deployed and `/health` sat on the previous commit for 200 s while I polled. Needed an empty trigger commit (`5cde59a`, same tree) to ship it. **Rule: the skip tag must never appear literally in a commit message unless you mean it — describe it as "the skip-render tag", never by its exact text.** The same applies to `[skip ci]`. (This docs-only commit carries the tag ON PURPOSE: CLAUDE.md is never read by the service, and a redeploy for it is the waste the tag exists to prevent.)

### 🔴 THE LAST STAGE IS MEASURED: Claude's SELECTION adds no detectable value (Sep 13)
The pick ledger cleared the honesty gate (n=48 matured; 46 picked vs 41 runners-up scored, 30-day horizon, `mtm_pct` — the one basis fair to controls, which carry no levels):

| | n | mean | sd |
|---|---|---|---|
| picked | 46 | **−0.01%** | 10.2 |
| not picked (runners-up) | 41 | **+4.73%** | 24.8 |

**Difference −4.74 pts, 95% CI [−12.89, +3.41], z = −1.14.** The interval includes zero — **not conclusive alone**. On alpha vs SPY: −5.51 pts, CI [−13.65, +2.63]. The controls' SD is 2.4× the picks' — a few big runner-up winners drive the gap, so read the *direction*, not the magnitude.
- **What IS established: all four stages of the engine are now measured and none shows a detectable edge** — ST ranking −1.6 (n=510), LT ranking −2.2 (n=90), eligible pool vs SPY −3.6, selection −4.74. Each is individually within noise; all four point the same way. Win rate 43.5% (CI 30–58%), 43.5% of picks beat SPY. **"These picks beat the market" is not a claim the evidence supports, at any stage.** That was written on Aug 24 as three-for-three; it is now four-for-four.
- **Do not tune anything on this** — n≈45/side cannot separate a real 3-point edge from noise, and the report says so. What it CAN say is that no stage has shown value yet, which is a reason to reconsider the product claim, not to move weights.
- **🔴 `--report-only` is NOT silent — it SENT a Telegram report.** It gates the ledger WRITE (`if not args.report_only`) but not the SEND; `--quiet` is the silencer. I sourced `.env` for the Gist creds, which also handed it the bot token, and the owner got a duplicate of Saturday's report. **"Read-only" ≠ "side-effect-free": before running any script with `.env` sourced, `unset TELEGRAM_BOT_TOKEN TELEGRAM_CHAT_ID` unless a send is the point.** Same family as the monkeypatch that wrote to the live gist.
- **Local gotcha: `evaluate_picks.py` reads `GIST_ID`/`GH_GIST_TOKEN` from the environment and does NOT `load_dotenv`** — run bare, it reports `ledger=0 rows` and *"No picks have matured yet"*. **That is a READ FAILURE, not an empty ledger** (the wrong-store trap, fifth time). The ledger genuinely lives on the Gist, so `set -a; . ./.env; set +a` with `SUPABASE_*` unset IS the right store here.

### 🔴 CI has been RED since Sep 7 — the merge gate is dead again, same class as Aug 24 (found Sep 13)
- `tests/test_selfheal_triage.py` (added Sep 6) has a module-level `import yaml`; **`pyyaml` is not in `requirements.txt`**. `test.yml` runs plain `pytest tests/ -q` with no `--continue-on-collection-errors`, so ONE unresolvable top-level import aborts collection of the WHOLE suite. **Eight consecutive red Tests runs (Sep 7–14), including every commit of Sep 13 — local "2061 passed" runs meant nothing on CI**, and self_heal's gate (same command) could not pass. Aug 24 was a missing test RUNNER; this is a missing test DEPENDENCY.
- **self-heal diagnosed it correctly on Sep 7 (`auto/self-heal-1788709064`): adds `pyyaml>=6.0` + `tests/test_no_missing_test_deps.py`, an AST scan that every test module's top-level imports are importable.** It sat unmerged for 7 days. Diagnosis right, delivery failed — now **10 of 10** across three incidents. **Merge it from `/admin`; CI going green is the verification.**
- **Rule: a test file's top-level `import X` requires `X` in `requirements.txt` in the SAME commit** — or import it function-locally, which only fails that one test instead of aborting collection. **A green local suite is not a green CI suite; check the Tests workflow before believing a run.**

### 🔴 The bot now OBEYS the entry window it publishes — and a skip is an OBSERVATION (Sep 13)
- **The defect**: 10 of 98 fills breached the published window, up to **11.22% (DOT 2026-09-08)** and **8.98% (NVDA 2026-08-27)**. The message says *"enter within X% — skip if above $Y"*, so the bot modelled a user who IGNORES the instruction, and every one of those fills is a pick a real obedient user would not have owned.
- **🔴 The finding's OWN suggested fix was wrong, and checking the data is what showed it.** It proposed widening `formatters.entry_window_pct` "to match measured reality". DOT is in the canonical crypto map, so this is not the bare-symbol pricing bug — an 11% overnight crypto move and an earnings gap are REAL. Widening would legitimise the bad fill and quietly weaken the promise. **`scripts/synthetic_user._entry_breach()` obeys the rule instead.** *A worklist item is a hypothesis, not an instruction* — the same call as the flat stop-tightness threshold.
- **Only ABOVE breaches** (a cheaper fill is a better one) and an **unusable entry returns None** — unmeasurable is never a violation, and treating it as one would STARVE the bot, which is indistinguishable from a quiet market. Imports the ONE window definition; a re-hardcoded `2`/`3` fails a test.
- **🔴 THE HALF THAT IS EASY TO MISS: simply not buying would have BLINDED the metric that found this.** `actionability` measures reachability from the bot's FILLS, so obeying the rule makes the breach rate fall toward 0% while nothing improved — the exact flattering-direction failure that let a closed position erase the worst breach on record (COHR 15%→8.7%; *"a metric about a PROMISE must not get quieter"*). So every skip is **persisted to `synthetic_state_*.json` and counted by `entry_slippage(..., skipped=)`** as a breach observation with `fill: None`. It is also strictly BETTER evidence — it is the obedient user's outcome, which is the thing being measured. Deduped on `(date, ticker)` because reachability is a property of the PICK, and a fill plus a skip for the same pick must count once.
- Skips are stamped with **`et_today()`** — actionability joins by DATE and a UTC stamp rolls over at 7-8 PM ET (the clock class, sixth occurrence). Bounded to the last 500, because the gist has a ~1 MB wall.
- **The prefetch guard written hours earlier CAUGHT THIS CHANGE** — `/admin/data` began reading `synthetic_state_900000001.json` and the test named the exact file. That is what a structural guard is for; the filename is now derived from `DEFAULT_TEST_CHAT_ID`, never hardcoded.
- **Verified by DRIVING it, not only in tests**: `phase_open(dry=True)` against live picks (nothing breached — correct), then re-run with every price +12% (the DOT shape) → **2 skips, 0 buys.** An untriggered branch is an untested one. Guards: `tests/test_bot_obeys_entry_window.py` (16), 5 mutations verified failing incl. the metric going quiet and a fill+skip double-count.
- **⚠️ The 5 historical ACT findings do NOT disappear** — they are past fills and cannot be un-made. They stay `open` until acknowledged on `/admin`. **Acknowledging them is the owner's call, and only after a clean run proves the fix holds.**

### ✅ CI green again (Sep 13) — and the `/admin` MERGE BUTTON silently did nothing
- **Seven days red** (Sep 7–14, every push). `tests/test_selfheal_triage.py` imports `yaml` at module level and `pyyaml` was absent from `requirements.txt`; `pytest tests/ -q` has no `--continue-on-collection-errors`, so ONE unresolvable top-level import aborted collection of the WHOLE suite — which is also self_heal's own merge gate. **Aug 24 was a missing test RUNNER; this was a missing test DEPENDENCY.** Same outcome: the gate could not pass.
- **Verified by reproducing CI, not by trusting a local run**: fresh venv, `pip install -r requirements.txt pytest`, then `--collect-only` → **2087 collected** (was: abort). The merge commit's Tests run is **green**. A local suite passing proves nothing about CI when the difference IS the dependency set.
- **🔴 THE OWNER CLICKED MERGE AND NOTHING HAPPENED.** `main` was unchanged, the branch still existed (so Discard had not fired either), and `git merge-tree` showed the merge was **CLEAN — no conflict**, with `requirements.txt` untouched on main since the merge-base. The endpoint is designed to alert loudly on a non-2xx, so either it returned a failure the owner did not read, or the click never reached it. **Root cause NOT established — do not write one down.** Landed via `git merge --no-ff` instead so CI was unblocked.
- **What to check when it recurs** (the branch was 1 ahead / **25 behind**, `status=diverged`): GitHub's `POST /repos/{repo}/merges` returns **409 on conflict, 403 on permission** — and merge scope was PROVEN on Sep 3 (`06bb7cc`), so a 403 would mean the token changed. Reproduce with the browser console open and read the alert text; that string is the answer.
- **Self-heal's record is now 11 correct diagnoses, 0 successful deliveries across four incidents** (unproposable gate → orphaned branches → branch churn → a merge button that did not fire). **The engine has never been the weak link. Delivery has failed every single time**, and each fix has addressed a different link in that chain.

### 🔴 The worklist was INVISIBLE on the surface built to decide it (Sep 13)
- Owner: *"acknowledge the 5 findings on admin"* — **and the card could not render them.** `_apply_state` writes a disposition record for EVERY derived finding, but an untouched one has `status: "open"`, and the card filtered to `awaiting_approval | approved | resolved_UNAPPROVED`. So the five ACT findings existed in the store, appeared in `ENGINE_FINDINGS.md`, and were **absent from `/admin`** — the one place built for ruling on them. Worse than the Aug 23 empty card: the owner was directed to a button that did not exist.
- **`"open"` is now rendered. Decided items stay hidden — the cry-wolf rule is unchanged**, and that distinction is the whole point: open items ARE the worklist; acknowledged/wont_fix ones are noise.
- **An open finding offers "Acknowledge" and "Won't fix", NEVER "Approve".** Nothing has been PROPOSED for it, so there is no concrete change to consent to — `admin_finding_disposition` returns **409** for exactly that case, so an Approve button there would be a control that cannot work. Guard pins that `'approved'` never appears in the open-status branch.
- **`x.title` is the readable fallback.** With nothing proposed, `proposed_summary`/`proposed_change`/`note` are all empty, so every worklist row would have read *"(no description recorded)"* — the unreadable-proposal failure one step earlier in the flow.
- **`acknowledged` is safe against the consent invariant**: `_apply_state` converts only `awaiting_approval` → `resolved_UNAPPROVED` on disappearance; everything else → `resolved`. Acknowledging cannot manufacture a false "built without approval" alarm.
- **⚠️ I could not do this for the owner even if it were appropriate** — `scripts/findings.py` has NO acknowledge verb (propose/approve/reject/reclassify/status), `findings.yml` offers only four actions, and a local run resolves to the **GIST**, the rollback copy, because `.env` carries no `SUPABASE_*`. **The only correct surfaces are `/admin` (owner's browser) and the workflow.** Guards: `TestOpenFindingsCanBeDecidedOnAdmin` (7), 3 mutations verified failing, plus a check that the markup reaches the SERVED page — source scans were what let the whole page 500 for 40 minutes.

### 🔴 ROOT CAUSE of the silent Merge button: a blocking `confirm()` (Sep 13)
- **The narrowing that found it**: the owner acknowledged 5 findings on `/admin` and the card went empty — so **POSTs, auth, the store and `load()` all work.** The failure was specific to ONE handler. Diffing them:

      setFinding   -> straight to fetch                  ✅ worked
      selfhealAct  -> if(!confirm(msg)) return;          ❌ silent return

- **`confirm()` returning falsy is the ONLY silent path in that function** — everything downstream alerted loudly. A browser that suppresses dialogs (Chrome offers exactly that after repeated ones) or **Telegram's in-app browser, where `confirm` is blocked outright**, makes it return false. Click, nothing, no trace: `main` unchanged, branch untouched, merge provably clean.
- **This repo already knew.** *"`window.confirm()` and `window.alert()` are blocked — always use `tg.showPopup()`"* was written for the mini-app; I used a bare `confirm()` on `/admin` as the ONLY gate on a **production deploy button**. The lesson was one surface away and I did not carry it across.
- **Fix: two-step IN-PAGE arming.** First click re-labels the button `Confirm — deploys now` and explains what happens; a second click within 8s fires; it disarms itself after. **A browser cannot suppress it.** Friction is preserved — it is still two deliberate actions.
...
- ✅ **THE HANDLER IS PROVEN (2026-09-18), and the proof is a FAILURE — which is the point.** First exercise since the fix: the owner clicked Merge and the card rendered, in red, in-page: `FAILED (HTTP 502): GitHub refused the merge (HTTP 409) {"message":"Merge conflict"…}`. POST reached GitHub, GitHub answered, the answer was surfaced with its own text. **Compare that to the defect it replaced: click, nothing, no trace.** A loud refusal is the designed outcome; do not "fix" it.
  🔎 The conflict was REAL and self-inflicted: `auto/self-heal-1789558366` adds a `_now_et` helper that `f03da25` had already landed on main WITH its caller, so `git merge-tree` exits 1 on the same region. **Verified before advising a discard** — the branch offers nothing main lacks.
  ✅ **DISCARD is proven too (2026-09-18) — it had never been exercised either.** The owner clicked it and the ref is gone; confirmed by read-back using `if out=$(git ls-remote …)`, because **a FAILED ls-remote also returns zero lines** and would otherwise masquerade as "0 branches remain" — the trap this file already records from the Aug-24 cleanup.
  ⏳ **Still unobserved: a SUCCESSFUL merge through the handler.** Sep 3's merge predates the `confirm()` bug. That needs a mergeable branch; keep the watch open rather than calling the loop closed.
...
### 🔴 Somebody waited TEN DAYS for access — `canary.check_pending_approvals()` (Sep 14)
- **The notification WORKED.** A DM with one-tap Approve/Reject fires the moment someone requests access. It was sent, missed, and **never mentioned again** — the same one-shot failure as the unmerged self-heal branches and the awaiting findings, both of which already had standing reminders here. Pending approvals had none.
- **This one costs more than those two.** The requester is TOLD *"usually within a few hours"*, so the broken promise lands on a stranger trying the product, not on the owner. That is the only one of the three a real user experiences.
- **MUST NEVER FAIL, for two reasons**: a canary failure triggers self_heal (which writes a branch), and **self-heal cannot approve a user** — no code change can make it pass, so it would burn credits diagnosing something uncodeable. Also listed in `owner_only_checks.OWNER_ONLY_CHECKS` as defence in depth.
- **Three traps it had to avoid, all previously paid for:**
  - **`read_strict`, never `get_pending_users()`** — that helper ends in `or {}`, so an unreadable store would report a clean *"nobody waiting"*. The exact false pass `check_findings_awaiting` shipped with.
  - **Tombstones are not people.** On a row backend a removal writes an EMPTY record rather than deleting the row; counting those would report somebody approved days ago as still waiting. Mirrors `get_pending_users`'s own filter.
...
✅ **STARTED 2026-09-20 — cron-job.org job `8479070` `StockPulz-arms-open`**, 8:05 AM ET Mon-Fri, ET-anchored (8 AM ET is the SAME calendar day in UTC, so no weekday can shift — the trap that cost Monday its prescreener for eight weeks; `audit_cron_schedules.py` clean). Auth header COPIED programmatically from job 8449904 and never printed; token `sha256[:16] 63a73f0a…` matches the other 20 jobs. Verified by READ-BACK, not by the HTTP 200. **NOT test-run on purpose: a cron-job.org TEST RUN is a real production run** and would make the arms buy off-schedule — the exact defect Phase 1 fixed.

⏳ The `open` phase is **not on GitHub's scheduler**, deliberately: GitHub runs this repo
