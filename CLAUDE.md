...
- Third leg alongside canary (verifies MATH/delivery) and synthetic-user (real lifecycle): this one verifies the **whole user SURFACE responds without 500/crash**. In ONE run it drives every GET `/api/miniapp/*` (read-only), a snapshot-safe pass over every MUTATING POST endpoint (positions/alerts/watchlist/settings/paper/feedback), every informational **bot command** (via `handle_incoming_command`), the inline-button **callback dispatcher** (via `handle_callback_query`), and the **LLM** endpoint (`/define`, every run per owner's choice). Scheduled by `.github/workflows/full_sweep.yml` ~3×/day (13:00/17:00/20:30 UTC).
...
- **Telegram muter**: `_mute_telegram()` monkeypatches `requests.post/get/Session.request` to stub any `api.telegram.org` URL (returns a fake 200) so driving dozens of commands never DMs the admin; Gist/github.com + price APIs pass through. Only the final report sends for real. NOTE: `broadcast_all` uses aiohttp (not requests) so it would bypass — the sweep deliberately never triggers broadcast paths.
- **Snapshot→restore in `finally`** over `_SNAP_FILES` (adds `feedback.json` + `backtest_trades.json` + `buy_counts.json` to canary's set) so every mutation is byte-reverted. Settings POSTs write fields back to their CURRENT value (no net change). `seed_backtest` is deliberately NOT exercised (async + writes many simulated trades).
- **Reporting**: loud DM on ANY failure (forward-to-Claude); on green stays silent except one end-of-day summary (`utcnow().hour >= 20`) → ~1 green DM/day. **Self-retires** after `_SWEEP_END` (2026-07-06) — delete the workflow after the window. **Rule: this is a temporary hardening tool; when a real endpoint/command/callback is added, it belongs in the permanent canary, not here.**
