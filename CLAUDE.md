...
- **Pick cards reflect alert STATE.** `_pickAlertBtnHtml(sym, entry)` renders `🔔 Alert $X` (amber) when an alert exists for the ticker, else `📡 Set Alert` (green); `_loadPickAlerts()` fetches `/api/miniapp/alerts` on picks-tab load (the watchlist tab already did) into `_watchAlertsData` and repaints every `[id^="alert-btn-"]` in place (no full re-render). One alert per ticker: `saveWatchAlert` removes the old target before adding a new one (edit = replace, not stack); the overlay's `🗑 Remove alert` button (`removeWatchAlert`, shown only when editing) resets it. Both refresh via `_loadPickAlerts()`. **Rule: after any alert add/remove, call `_loadPickAlerts()` so the pick button flips state.**
