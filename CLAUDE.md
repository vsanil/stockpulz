...
- **FULL AUTO-MERGE + DEPLOY on green (owner's explicit choice, Jul 29).** If `pytest tests/ -q` AND `check_js.py` both pass, it **pushes the commit straight to `main`** (`git push origin HEAD:main`) → **Render `autoDeploy` ships it live in ~1-2 min**. **NOT via a PR** — `GITHUB_TOKEN` can't create PRs by default ("GitHub Actions is not permitted to create or approve pull requests"), and full-auto doesn't need one. A **post-deploy `/health` smoke-check** DMs a loud `🚨` if the deploy broke the app (roll back in Render). DMs the admin `🔧 fixing` at start and `✅ merged & deployed` / `🚨 health down` / `⚠️ pushed to branch (tests red)` / `🧪 test fix` / `⚠️ couldn't fix` at the end.
