...
- **Triggers on `workflow_run` failure of the three monitors** (matched by NAME: "Daily Canary", "Synthetic User", "Full Sweep" — NOT filename; the monitors must be on `main` for `workflow_run` to fire). Runs **Claude Code headless** (`anthropics/claude-code-action@v1`, `--model claude-sonnet-5 --max-turns 12 --allowedTools "Read,Edit,Write,Bash"`) to diagnose + fix on a new branch. The prompt tells it to read `/tmp/failure.log` + CLAUDE.md, classify FALSE-ALARM (fix the check in `scripts/`) vs REAL BUG (fix app code + add a test), and make NO commit if not confident.
