...
- **Pick-sheet alert is a CUSTOM price**, not fixed-at-entry. The mini-app pick sheet's "📡 Set Alert" calls `openWatchAlert(sym, entryPrice)` (the shared watchlist overlay), which now takes an optional `defaultPrice` and guards `(_watchAlertsData || [])` so it works cross-tab. The old one-tap `setPickEntryAlert` (fixed at entry) was removed. **Rule: reuse `openWatchAlert` for any per-ticker alert UI — one overlay, custom price.**
