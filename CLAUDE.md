...
- **Triggers on `workflow_run` failure of the three monitors** (matched by NAME: "Daily Canary", "Synthetic User", "Full Sweep" — NOT filename; the monitors must be on `main` for `workflow_run` to fire). Runs **Claude Code headless via the CLI** (`npm i -g @anthropic-ai/claude-code`, then `claude -p "$(cat /tmp/prompt.txt)" --model claude-sonnet-5 --max-turns 14 --dangerously-skip-permissions`) to diagnose + fix on a new branch. **Deliberately NOT `anthropics/claude-code-action@v1`** — that action requires the Claude Code GitHub App installed on the repo (an owner-only browser step); the CLI authenticates with just `ANTHROPIC_API_KEY` and needs no app. The prompt tells it to read `/tmp/failure.log` + CLAUDE.md, classify FALSE-ALARM (fix the check in `scripts/`) vs REAL BUG (fix app code + add a test), and make NO commit if not confident.
- **FULL AUTO-MERGE + DEPLOY on green (owner's explicit choice, Jul 29).** If `pytest tests/ -q` AND `check_js.py` both pass, it squash-merges to `main` → **Render `autoDeploy` ships it live in ~1-2 min**. Then a **post-deploy `/health` smoke-check** DMs a loud `🚨` if the deploy broke the app (roll back in Render). DMs the admin `🔧 fixing` at start and `✅ merged & deployed` / `🚨 health down` / `⚠️ draft PR` / `⚠️ couldn't fix` at the end.
- **The 676-test suite is now the ONLY gate between an LLM's fix and your live users.** So: **never weaken/skip the test gate, and any new user-facing calc/feature MUST have test coverage** — an uncovered bug can now auto-deploy. A wrong fix reaching prod is the accepted risk of full-auto (owner chose it over PR-for-approval after being warned twice).
- **A MANUAL `workflow_dispatch` NEVER auto-merges** (opens a PR instead) — `AUTO_MERGE` is gated on `github.event_name == 'workflow_run'`. Use `gh workflow run self_heal.yml -f failed_run_id=<id>` to validate the loop safely (point at an already-fixed failure → Claude makes no change → machinery-only test). To revert to PR-for-approval: set `AUTO_MERGE: 'false'`. To stop entirely: disable/delete the workflow.
