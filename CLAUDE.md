...
- **RESIDUAL (documented, deliberate):** `add_holding`, the paper_trader paths, and the two `agent.py` job sites still use load→modify→save (their prep work is interleaved with the mutation, so converting them is a money-path refactor not worth doing days before launch). The window is milliseconds-to-seconds, not eliminated. **The real fix is row-level storage — `supabase` is already a dependency;** migrating user records off the Gist would remove this class entirely.
